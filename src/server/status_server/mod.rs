...
#[derive(Default)]
struct BusyThreads<Workload> {
    threads: Vec<std::sync::mpsc::Sender<()>>,
    workload: Workload,
}

impl<Workload: BusyWorkload + 'static> BusyThreads<Workload> {
    fn busy(&mut self, n: usize) {
        for i in 0..n {
            let (sx, rx) = std::sync::mpsc::channel();
            let workload = self.workload.clone();
            std::thread::Builder::new()
                .name(format!("busyworker-{}", i + self.threads.len()))
                .spawn(move || {
                ...
                .unwrap();
            self.threads.push(sx);
        }
    }

    fn stop(&mut self) {
        self.threads.drain(..).for_each(|sx| sx.send(()).unwrap())
    }
}

impl BusyThreads<NopWorkload> {
    fn new_noop() -> Self {
        Self::default()
    }
}

trait BusyWorkload: Clone + Send {
    fn run(&self);
}
...
impl BusyWorkload for NopWorkload {
    fn run(&self) {}
}

#[derive(Clone)]
struct ScanWorkload<E, R> {
    engine: E,
    region_info: R,
    select_region_by: Vec<u8>,
}

impl<E: Engine, R: RegionInfoProvider + Clone + 'static> BusyWorkload for ScanWorkload<E, R> {
    fn run(&self) {
        //TODO: implement
    }
}
    ...
    R: 'static + Send + CasualRouter<E> + Clone,
{
        ...
        C: ServerConnection,
    {
        ...
        let busy_threads = Arc::new(std::sync::Mutex::new(BusyThreads::new_noop()));
        // Start to serve.
                            ...
                            (Method::PUT, "/debug/busy") => {
                                let n = req
                                    ...
                                    .and_then(|query| {
                                        let query_pairs: HashMap<_, _> =
                                            url::form_urlencoded::parse(query.as_bytes()).collect();
                                        query_pairs.get("n").and_then(|n| match n.parse() {
                                            Ok(n) => Some(n),
                                            Err(_) => None,
                                        })
                                    })
                                    .unwrap_or(4);
                                busy.lock().unwrap().busy(n as _);
                                Ok(Response::default())
                            }
                            (Method::DELETE, "/debug/busy") => {
                                busy.lock().unwrap().stop();
                                Ok(Response::default())
                            }
